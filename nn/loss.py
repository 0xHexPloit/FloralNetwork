...
def loss_discriminator(predictions_fake, predictions_true):
    ...
    loss_fake = bce_loss(predictions_fake, target_fake)

    target_true = torch.zeros_like(predictions_true)
    loss_true = bce_loss(predictions_true, target_true)
...
def loss_generator(predictions, generated_img, true_img):
    ...
    return bce_loss(predictions, target) + config.LAMBDA_PARAM * l1_loss(generated_img, true_img)
